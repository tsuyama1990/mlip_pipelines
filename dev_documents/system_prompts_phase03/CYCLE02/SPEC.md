...
## Summary
CYCLE02 focuses on the "Exploration" phase of the Active Learning loop. Instead of relying on naive random sampling, the system implements an intelligent `AdaptiveExplorationPolicyEngine` that determines the optimal sampling strategy (e.g., Temperature schedules, MD vs. MC ratios, strain limits) based on the intrinsic properties of the material (e.g., metals vs. insulators).

Crucially, this cycle implements the "Initial Exploration (Cold Start)" mechanism. Before the first policy decision, the system attempts to autonomously deduce the required material properties (e.g., $E_g$, $T_m^{pred}$, $B_0$) using a universal potential API (like M3GNet or CHGNet) via a `FeatureExtractor` module. If these heavy dependencies are unavailable, it falls back to a robust rule-based mock, preserving the Zero-Config principle.

Additionally, the `StructureGenerator` is implemented to provide the physical candidate structures. It creates synthetic interfaces (like FePt/MgO) at initialization and generates localized perturbed candidate structures (via rattling) around highly uncertain atomic environments detected during the MD/kMC simulation.
...
│   ├── **cold_start.py**     (Implements FeatureExtractor using universal potentials/mocks)
...
**Key Interfaces:**
- `FeatureExtractor`: Uses M3GNet/CHGNet APIs (or rule-based mocks) to populate the `MaterialFeatures` DTO autonomously.
- `AdaptiveExplorationPolicyEngine`: Evaluates `MaterialFeatures` to output an `ExplorationStrategy`.
...
1.  **Cold Start Feature Extraction**: The `FeatureExtractor` (`cold_start.py`) addresses the "Cold Start" requirement. It takes the user's base input (just the element symbols) and queries a universal model. To prevent the entire AL loop from breaking if a universal model is not installed locally, the extractor uses a `try-except ImportError` block to gracefully degrade into a deterministic, rule-based fallback that predicts properties based on the periodic table (e.g., assuming transition metals are conductors).
2.  **Adaptive Exploration Policy**: The policy engine (`adaptive_policy.py`) consumes the `PolicyConfig` (defining default scales and ratios) and the `MaterialFeatures` generated by the `FeatureExtractor`. It outputs a strongly-typed `ExplorationStrategy` DTO containing precise numerical parameters for the next simulation run (e.g., `t_max`, `md_mc_ratio`).
...
1.  **Implement Feature Extractor (`generators/cold_start.py`)**:
    -   Implement a `FeatureExtractor` class with a `extract_features(elements: list[str]) -> MaterialFeatures` method.
    -   Attempt to import `matgl` (M3GNet) or `chgnet`. If successful, instantiate a small bulk cell and perform a fast static prediction to estimate energy, band gap, and bulk modulus.
    -   Implement a fallback `_mock_extraction(elements)` method that uses hardcoded heuristics (e.g., if 'Fe' in elements, $E_g = 0.0$, $T_m = 1811$K) to guarantee the `MaterialFeatures` DTO is successfully populated even in constrained CI environments.
2.  **Implement Adaptive Policy Engine (`generators/adaptive_policy.py`)**:
...
-   **Feature Extraction Fallback**: Write a test for `FeatureExtractor.extract_features()` ensuring that even when `matgl`/`chgnet` is uninstalled (simulated via `sys.modules`), it returns a fully valid `MaterialFeatures` DTO by utilizing the periodic-table fallback logic.
