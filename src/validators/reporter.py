...
class Reporter:
    ...
    def generate_html_report(self, report: ValidationReport, save_path: Path) -> None:
        ...
        reason_html = f"<p>Reason: {report.reason}</p>" if report.reason else ""
        phonon_text = "Yes" if report.phonon_stable else "No"
        mech_text = "Yes" if report.mechanically_stable else "No"
