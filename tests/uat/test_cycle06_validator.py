...
def test_uat_06_01_quality_assurance_pass(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        sys.modules["pyacemaker.calculator"],
        "pyacemaker",
        lambda path: MockCalc(energy=1.001, forces=np.array([[0.1, 0.1, 0.1]])),
    )

    config = ValidatorConfig(
        energy_rmse_threshold=0.002,
        force_rmse_threshold=0.05,
...
def test_uat_06_02_quality_assurance_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        sys.modules["pyacemaker.calculator"],
        "pyacemaker",
        lambda path: MockCalc(energy=1.0, forces=np.array([[0.3, 0.3, 0.3]])),
    )
