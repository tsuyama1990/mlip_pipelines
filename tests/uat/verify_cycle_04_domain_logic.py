...
def __() -> tuple:
    ...
    # RUF001 NOQA wrapper
    return DynamicsConfig, SystemConfig, MDInterface, DynamicsHaltInterrupt, Path, logging, subprocess, shutil, Any, pytest

@app.cell
def test_scenario_01(DynamicsConfig: Any, SystemConfig: Any, MDInterface: Any, Path: Any, subprocess: Any, shutil: Any, Any: Any) -> tuple:
    ...
    with _tempfile.TemporaryDirectory() as _td:
        ...
        print("✓ UAT-C04-01 & UAT-C04-03 passed. Resume script correctly injects soft-start langevin protocol and two-tier thresholds.")
...
@app.cell
def test_scenario_02(DynamicsConfig: Any, SystemConfig: Any, MDInterface: Any, DynamicsHaltInterrupt: Any, Path: Any, subprocess: Any, shutil: Any, Any: Any) -> tuple:
    ...
    with _tempfile.TemporaryDirectory() as _td:
        ...
        print("✓ UAT-C04-02: Correctly parsed AL_HALT as an expected exception, returning halted=True.")
        ...
        try:
            ...
            print("✓ UAT-C04-02: Correctly recognized missing AL_HALT as a fatal physical crash, raising DynamicsHaltInterrupt.")
...
def test_scenario_04(DynamicsConfig: Any, SystemConfig: Any, MDInterface: Any, Path: Any) -> tuple:
    ...
    with _tempfile.TemporaryDirectory() as _td:
        ...
        # THEN
        assert "read_data" not in _script # Note: write_cold_start actually doesn't use read_data natively, it generates the box internally via `create_atoms` for our architecture, which perfectly lacks read_restart. Let's verify `read_restart` is missing.
        assert "create_atoms" in _script
        assert "read_restart" not in _script
        assert "fix soft_start" not in _script
        print("✓ UAT-C04-04 passed. Initial phase correctly generates a cold-start script missing read_restart and soft_start constraints.")
