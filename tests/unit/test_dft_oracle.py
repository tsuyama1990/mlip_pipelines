...
def test_dft_oracle_compute_batch(mock_which: MagicMock, tmp_path: Path) -> None:
    ...
    with patch("ase.calculators.espresso.Espresso.get_potential_energy") as mock_pe, \
         patch("ase.calculators.espresso.Espresso.get_forces", return_value=np.zeros((2, 3))) as mock_forces:
        ...
        with patch("src.oracles.dft_oracle.Espresso") as MockEspresso:
            ...
            with patch("ase.Atoms.get_potential_energy", return_value=-100.0), \
...
def test_dft_oracle_compute_batch_exception_handling(mock_which: MagicMock, tmp_path: Path) -> None:
    ...
    with patch("src.oracles.dft_oracle.Espresso") as MockEspresso:
        ...
        with patch("ase.Atoms.get_potential_energy", side_effect=[Exception("mocked error"), -100.0]), \
             patch("ase.Atoms.get_forces", return_value=np.zeros((2, 3))):
