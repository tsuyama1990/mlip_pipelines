...
def test_distillation_config_path_validation(tmp_path: Path) -> None:
    ...
    # Valid path (must exist now)
    valid_model = (tmp_path / "my_model.pt").resolve(strict=False)
    with open(valid_model, "w") as f:
        f.write("dummy")

    assert valid_model.exists()
    assert valid_model.is_file()

    config = DistillationConfig(mace_model_path=str(valid_model))
    assert config.mace_model_path == str(valid_model)
    ...
    with pytest.raises(ValidationError, match="Model file does not exist or is not a file"):
        DistillationConfig(mace_model_path="/tmp/does_not_exist_ever_model.pt")
